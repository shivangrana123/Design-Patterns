...
## 1) Intent
Encapsulate a request as an object so actions can be executed later, logged, or undone.

## 2) Problem it solves
UI code or API endpoints become tightly coupled to business operation implementations.

## 3) Core idea
Represent each action as a command object with `Execute()`.

## 4) Participants in this example
ICommand, LightOnCommand, Light, RemoteControl

## 5) Step-by-step flow
- Wrap each action in a command class.
- Invoker receives a command and triggers execution.
- Receiver performs the real work.
- Store command history for auditing or undo.
