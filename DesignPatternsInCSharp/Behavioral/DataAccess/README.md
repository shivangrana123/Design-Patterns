...
## 1) Intent
Separate persistence concerns from business logic with repository abstractions.

## 2) Problem it solves
Domain code gets mixed with SQL/ORM details and becomes hard to test.

## 3) Core idea
Expose repositories/UoW interfaces and hide data source details.

## 4) Participants in this example
IUserRepository, InMemoryUserRepository, UserService

## 5) Step-by-step flow
- Create repository interface around aggregate operations.
- Implement persistence details behind interface.
- Inject repository into application/domain services.
- Swap infrastructure without changing business rules.
