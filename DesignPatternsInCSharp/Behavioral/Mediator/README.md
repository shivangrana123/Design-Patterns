...
## 1) Intent
Centralize communication logic between collaborating objects.

## 2) Problem it solves
Objects talk to each other directly, creating many dependencies.

## 3) Core idea
Route interactions through a mediator that coordinates participants.

## 4) Participants in this example
IChatMediator, ChatRoomMediator, User

## 5) Step-by-step flow
- Define mediator interface for communication rules.
- Register participants with mediator.
- Participants send messages via mediator.
- Mediator decides who receives what and when.
